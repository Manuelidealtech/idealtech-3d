...
export default function ShareManager({ products, links, appUrl }: { products: Product[]; links: ShareLink[]; appUrl: string }) {
  ...
  async function revoke(id:string){await fetch('/api/shares',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({id,active:false})});location.reload();}
